...
Vérifié : servi en HTTP, la vue par défaut affiche les 67 combinaisons **vérifiées** (0 non
...
Le jeu de données contient **67 combinaisons vérifiées** (4 marques : IKEA, Castorama/Atomia, Muuto/Stacked, HAY/Colour Cabinet) et **28 non vérifiées** (IKEA + HAY).
**Rien d'incertain n'est affiché comme certain.** Par défaut, **seules les 67 vérifiées sont
...
- **Plusieurs marques câblées** : `ikea.json` (53) + `marques-2026-09-14.json` (Castorama/Atomia 9 + Muuto/Stacked 3 + HAY/Colour Cabinet 2 vérifiées, + 4 HAY non vérifiées),
  importées dans `dataset.ts`. Un **filtre « Marque »** (facette `enseignes`, 4 marques) permet de distinguer et filtrer par marque.
  Cotes décimales réelles gérées (Atomia 37,5/187,5 ; Muuto 21,8/43,6/65,4 ; HAY profondeur 39) : bornes et sliders au **pas de 0,1 cm**, aucun arrondi.
